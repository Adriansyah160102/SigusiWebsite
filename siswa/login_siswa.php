...
<?php
    ...
    if (!empty($_GET['username'])){ $user=$_GET['user']; }else{ $user=''; }
    if (! empty($_GET['password'])) { $text=$_GET['pass']; } else { $text=''; }
?>
...
<head>
	...
	<title>Sigusi - Login Siswa</title>
</head>
<body onload="cekvalidasi('<?php echo $text; ?>');">
	<div class="container">
		<form action="cek_login.php" method="POST" class="login-email">
			<p class="login-text" style="font-size: 2rem; font-weight: 800;">Login Siswa</p>
